...
import { Rule, valueAs, optionalParameterConversion, Rules, defaultNameMapping, GenericConstructor, getRules } from './mapping.highlevel'
...
import Vector, { isVector, vector, VectorType } from './vector'
import { newError } from './error'
import Integer, { isInt } from './integer'
import UUID, { isUUID, uuid } from './uuid'
    ...
    return {
      validate: (value: any, field: string) => {
        ...
        if (rule?.dimension != null && value.asTypedArray().length !== rule.dimension) {
          throw new TypeError(`${field} should be a vector of length ${rule.dimension} but received length ${value.asTypedArray().length as string}`)
        }
