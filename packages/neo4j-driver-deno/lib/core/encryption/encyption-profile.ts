...
export class EnvelopeEncryptionProfile {
  ...
  private _keyCache: Map<string, CacheEntry<EncapsulatedKey>>
  private _aliasCache: Map<string, CacheEntry<string>>
  ...
  async saveKey(alias: string, encapsulation: Int8Array, metadata: Record<string, string>): Promise<EncapsulatedKey>{
    return this._keyRepository.save(alias, encapsulation, metadata)
  }
  async _checkKeyCache(id: string): Promise<EncapsulatedKey | undefined> {
    if (this._keyCache.has(id) === true) {
      let entry = this._keyCache.get(id)
      if (new Date().getTime() - (entry?.retrieved.getTime() ?? 0) < (this._keyCacheTTL ?? 0)) {
        return entry?.entry
      }
    }
    let key = await this._keyRepository.findById(id)
  ...
  async _checkAliasCache(alias: string): Promise<EncapsulatedKey | undefined> {
    if (this._aliasCache.has(alias) === true) {
      let entry = this._aliasCache.get(alias)
      if (new Date().getTime() - (entry?.retrieved.getTime() ?? 0) < (this._keyAliasCacheTTL ?? 0)) {
        // @ts-ignore
        return this._checkKeyCache(entry?.entry)
      }
    }
    let key = await this._keyRepository.findByAlias(alias)
