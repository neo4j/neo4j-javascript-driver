...
export default class HttpResponseConverter {
  ...
  extractError (response) {
    ...
    if (errors) {
      ...
      if (error) {
        ...
        const code = response.password_change
  ...
  extractTransactionId (response) {
    ...
    throw new Neo4jError(
      `Unable to extract transaction id from the response JSON: ${JSON.stringify(
        response
      )}`
    )
  }
...
function encodeQueryParameter (value) {
  if (value instanceof Node) {
    throw new Neo4jError(
      'It is not allowed to pass nodes in query parameters',
      PROTOCOL_ERROR
    )
  } else if (value instanceof Relationship) {
    throw new Neo4jError(
      'It is not allowed to pass relationships in query parameters',
      PROTOCOL_ERROR
    )
  } else if (value instanceof Path) {
    throw new Neo4jError(
      'It is not allowed to pass paths in query parameters',
      PROTOCOL_ERROR
    )
...
function newUnsupportedParameterError (name) {
  return new Neo4jError(
    `It is not allowed to pass ${name} in query parameters when using HTTP endpoint. ` +
      `Please consider using Cypher functions to create ${name} so that query parameters are plain objects.`,
    PROTOCOL_ERROR
  )
}
...
function extractRawRecord (data) {
  ...
  if (row) {
    return row.map((ignore, index) =>
      extractRawRecordElement(index, data, nodesById, relationshipsById)
    )
  }
  return []
}
  ...
  if (graph) {
    ...
    if (relationships) {
      return relationships.reduce((result, relationship) => {
        ...
        result[relationship.id] = new Relationship(
          identity,
          startNode,
          endNode,
          type,
...
function convertPath (metadata, nodesById, relationshipsById) {
  ...
  for (let i = 0; i < metadata.length; i++) {
    ...
    const nodeExpected =
      (startNode === null && relationship === null) ||
      (startNode !== null && relationship !== null)
    if (nodeExpected && elementType !== 'node') {
      throw new Neo4jError(
        `Unable to parse path, node expected but got: ${JSON.stringify(
          element
        )} in ${JSON.stringify(metadata)}`
      )
    }
    if (!nodeExpected && elementType === 'node') {
      throw new Neo4jError(
        `Unable to parse path, relationship expected but got: ${JSON.stringify(
          element
        )} in ${JSON.stringify(metadata)}`
      )
    }
    ...
    if (nodeExpected) {
      ...
      if (startNode === null) {
        ...
        throw new Neo4jError(
          `Unable to parse path, illegal node configuration: ${JSON.stringify(
            metadata
          )}`
        )
      }
      ...
      if (relationship === null) {
        ...
        throw new Neo4jError(
          `Unable to parse path, illegal relationship configuration: ${JSON.stringify(
            metadata
          )}`
        )
      }
  ...
  if (
    (lastPathSegment && lastPathSegment.end !== startNode) ||
    relationship !== null
  ) {
...
function convertPoint (element) {
  ...
  if (type !== 'Point') {
    throw new Neo4jError(
      `Unexpected Point type received: ${JSON.stringify(element)}`
    )
  }
  ...
  if (
    ...
    throw new Neo4jError(
      `Unexpected Point coordinates received: ${JSON.stringify(element)}`
    )
  }
...
function convertCrsToId (element) {
  ...
  if (!crs || !crs.name) {
    throw new Neo4jError(
      `Unexpected Point crs received: ${JSON.stringify(element)}`
    )
  }
