...
const TrustStrategy = {
  ...
  TRUST_SIGNED_CERTIFICATES: function (config, onSuccess, onFailure) {
    console.warn(
      '`TRUST_SIGNED_CERTIFICATES` has been deprecated as option and will be removed in a future version of ' +
        'the driver. Please use `TRUST_CUSTOM_CA_SIGNED_CERTIFICATES` instead.'
    )
    return TrustStrategy.TRUST_CUSTOM_CA_SIGNED_CERTIFICATES(
      config,
      onSuccess,
      onFailure
    )
  ...
  TRUST_ON_FIRST_USE: function (config, onSuccess, onFailure) {
    console.warn(
      '`TRUST_ON_FIRST_USE` has been deprecated as option and will be removed in a future version of ' +
    ...
    const socket = tls.connect(
      ...
      function () {
        ...
        if (!serverCert.raw) {
          ...
          onFailure(
            newError(
              'You are using a version of NodeJS that does not ' +
                ...
                'config instead, or disable encryption using `encrypted:"' +
                ENCRYPTION_OFF +
                '"`.'
            )
          )
        ...
        const knownHostsPath =
          ...
          path.join(userHome(), '.neo4j', 'known_hosts')
        ...
        loadFingerprint(serverId, knownHostsPath, knownFingerprint => {
          if (knownFingerprint === serverFingerprint) {
            ...
            onFailure(
              newError(
                'Database encryption certificate has changed, and no longer ' +
                  'matches the certificate stored for ' +
                  serverId +
                  ' in `' +
                  knownHostsPath +
                  ...
                  serverCert +
                  '. If you trust that this certificate is valid, simply remove the line ' +
                  'starting with ' +
                  serverId +
                  ' in `' +
                  knownHostsPath +
