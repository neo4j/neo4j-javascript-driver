...
// 64-bit Integer library, originally from Integer.js by dcodeIO
// https://github.com/dcodeIO/Integer.js
// License Apache 2
 ...
 * @constructor
 */
var Integer = function(low, high, unsigned) {
 ...
 * @expose
 */
Integer.fromInt = function(value, unsigned) {
    var obj, cachedObj;
    if (!unsigned) {
        value = value | 0;
        ...
        obj = new Integer(value, (value | 0) < 0 ? -1 : 0, true);
        if (0 <= value && value < 256)
 ...
 * @expose
 */
Integer.fromNumber = function(value, unsigned) {
    unsigned = !!unsigned;
    ...
    if (unsigned && value >= TWO_PWR_64_DBL)
        return Integer.MAX_UNSIGNED_VALUE;
    if (value < 0)
        return Integer.fromNumber(-value, unsigned).negate();
    return new Integer((value % TWO_PWR_32_DBL) | 0, (value / TWO_PWR_32_DBL) | 0, unsigned);
};
 ...
 * @expose
 */
Integer.fromBits = function(lowBits, highBits, unsigned) {
 ...
 * @expose
 */
Integer.fromString = function(str, unsigned, radix) {
    if (str.length === 0)
    ...
    if ((p = str.indexOf('-')) > 0)
        ...
        return Integer.fromString(str.substring(1), unsigned, radix).negate();
...
Integer.fromValue = function(val) {
    ...
    return new Integer(val.low, val.high, val.unsigned);
};
 ...
 * @expose
 */
Integer.UZERO = Integer.fromInt(0, true);
 ...
 * @expose
 */
Integer.UONE = Integer.fromInt(1, true);
 ...
 * @expose
 */
Integer.MAX_UNSIGNED_VALUE = Integer.fromBits(0xFFFFFFFF|0, 0xFFFFFFFF|0, true);
...
Integer.prototype.toString = function(radix) {
    ...
    // minimize the calls to the very expensive emulated div.
    var radixToPower = Integer.fromNumber(Math.pow(radix, 6), this.unsigned);
    rem = this;
    var result = '';
    while (true) {
 ...
 * @expose
 */
Integer.prototype.getHighBitsUnsigned = function() {
    return this.high >>> 0;
};
 ...
 * @expose
 */
Integer.prototype.getLowBitsUnsigned = function() {
    return this.low >>> 0;
};
...
Integer.prototype.negate = function() {
    if (!this.unsigned && this.equals(Integer.MIN_VALUE))
        return Integer.MIN_VALUE;
...
Integer.prototype.add = function(addend) {
    ...
    c48 += a48 + b48;
    c48 &= 0xFFFF;
    return Integer.fromBits((c16 << 16) | c00, (c48 << 16) | c32, this.unsigned);
};
...
Integer.prototype.multiply = function(multiplier) {
    ...
    if (this.lessThan(TWO_PWR_24) && multiplier.lessThan(TWO_PWR_24))
        return Integer.fromNumber(this.toNumber() * multiplier.toNumber(), this.unsigned);
...
Integer.prototype.div = function(divisor) {
    ...
    if (this.isZero())
        return this.unsigned ? Integer.UZERO : Integer.ZERO;
    var approx, rem, res;
    if (this.equals(Integer.MIN_VALUE)) {
    ...
    while (rem.greaterThanOrEqual(divisor)) {
        ...
        while (approxRem.isNegative() || approxRem.greaterThan(rem)) {
            approx -= delta;
            approxRes = Integer.fromNumber(approx, this.unsigned);
            approxRem = approxRes.multiply(divisor);
        }
...
Integer.prototype.not = function() {
    return Integer.fromBits(~this.low, ~this.high, this.unsigned);
};
...
Integer.prototype.and = function(other) {
    if (!Integer.isInteger(other))
        other = Integer.fromValue(other);
    return Integer.fromBits(this.low & other.low, this.high & other.high, this.unsigned);
};
...
Integer.prototype.shiftLeft = function(numBits) {
    ...
    if ((numBits &= 63) === 0)
        ...
        return Integer.fromBits(this.low << numBits, (this.high << numBits) | (this.low >>> (32 - numBits)), this.unsigned);
    else
        return Integer.fromBits(0, this.low << (numBits - 32), this.unsigned);
};
...
Integer.prototype.shiftRight = function(numBits) {
    ...
    if ((numBits &= 63) === 0)
        ...
        return Integer.fromBits((this.low >>> numBits) | (this.high << (32 - numBits)), this.high >> numBits, this.unsigned);
    else
        return Integer.fromBits(this.high >> (numBits - 32), this.high >= 0 ? 0 : -1, this.unsigned);
};
 ...
 * @expose
 */
Integer.prototype.shiftRightUnsigned = function(numBits) {
    if (Integer.isInteger(numBits))
        numBits = numBits.toInt();
    numBits &= 63;
    if (numBits === 0)
        ...
        if (numBits < 32) {
            var low = this.low;
            return Integer.fromBits((low >>> numBits) | (high << (32 - numBits)), high >>> numBits, this.unsigned);
        } else if (numBits === 32)
            return Integer.fromBits(high, 0, this.unsigned);
        else
            return Integer.fromBits(high >>> (numBits - 32), 0, this.unsigned);
    }
};
...
Integer.prototype.toSigned = function() {
    if (!this.unsigned)
        return this;
    return new Integer(this.low, this.high, false);
};
 ...
 * @expose
 */
Integer.prototype.toUnsigned = function() {
    if (this.unsigned)
        return this;
    return new Integer(this.low, this.high, true);
};
