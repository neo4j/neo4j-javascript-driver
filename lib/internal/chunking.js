...
class Chunker {
  ...
  putBytes ( position, data ) {
    ...
    while ( data.remaining() > 0 )
    {
      ...
      this._ensure( 1 );
      let slice = data.readSlice( Math.min( this._buffer.remaining(), data.remaining() ) );
  ...
  _ensure ( size ) {
    let toWriteSize = this._chunkOpen ? size : size + CHUNK_HEADER_SIZE;
  ...
  _closeChunkIfOpen () {
    if ( this._chunkOpen ) {
      let chunkSize = this._buffer.position - (this._currentChunkStart + CHUNK_HEADER_SIZE);
