...
Packer.prototype.pack = function(x) {
  if (x === null) {
    this._ch.writeByte( NULL );
  } else if (x === true) {
    this._ch.writeByte( TRUE );
  } else if (x === false) {
    this._ch.writeByte( FALSE );
    ...
    this.packStructHeader(x.fields.length, x.signature);
    for(var i = 0; i < x.fields.length; i++) {
      this._pack(x.fields[i]);
    }
...
Packer.prototype.packText = function(x) {
  ...
  if (size < 0x10) {
    this._ch.writeByte(TINY_TEXT | size);
    this._ch.writeBytes(bytes);
  } else if (size < 0x100) {
    this._ch.writeByte(TEXT_8)
    this._ch.writeByte(size);
    this._ch.writeBytes(bytes);
  } else if (size < 0x10000) {
    this._ch.writeByte(TEXT_16);
    this._ch.writeByte(size/256>>0);
    this._ch.writeByte(size%256);
    this._ch.writeBytes(bytes);
  } else if (size < 0x100000000) {
    this._ch.writeByte(TEXT_32);
    this._ch.writeByte((size/16777216>>0)%256); // TODO: Why is it shifting by 0 here?
    this._ch.writeByte((size/65536>>0)%256);
    this._ch.writeByte((size/256>>0)%256);
    this._ch.writeByte(size%256);
    this._ch.writeBytes(bytes);
  } else {
...
Packer.prototype.packListHeader = function(size) {
  if (size < 0x10) {
    this._ch.writeByte(TINY_LIST | size);
  } else if (size < 0x100) {
    this._ch.writeByte(LIST_8)
    this._ch.writeByte(size);
  } else if (size < 0x10000) {
    this._ch.writeByte(LIST_16, size/256>>0, size%256);
  } else if (size < 0x100000000) {
    this._ch.writeByte(LIST_32);
    this._ch.writeByte((size/16777216>>0)%256);
    this._ch.writeByte((size/65536>>0)%256);
    this._ch.writeByte((size/256>>0)%256);
    this._ch.writeByte(size%256);
  } else {
...
Packer.prototype.packMapHeader = function(size) {
  if (size < 0x10) {
    this._ch.writeByte(TINY_MAP | size);
  } else if (size < 0x100) {
    this._ch.writeByte(MAP_8);
    this._ch.writeByte(size);
  } else if (size < 0x10000) {
    this._ch.writeByte(MAP_16);
    this._ch.writeByte(size/256>>0);
    this._ch.writeByte(size%256);
  } else if (size < 0x100000000) {
    this._ch.writeByte(MAP_32);
    this._ch.writeByte((size/16777216>>0)%256);
    this._ch.writeByte((size/65536>>0)%256);
    this._ch.writeByte((size/256>>0)%256);
    this._ch.writeByte(size%256);
...
Packer.prototype.packStructHeader = function(size, signature) {
  if (size < 0x10) {
    this._ch.writeByte(TINY_STRUCT | size);
    this._ch.writeByte(signature);
  } else if (size < 0x100) {
    this._ch.writeByte(STRUCT_8);
    this._ch.writeByte(size);
    this._ch.writeByte(signature);
  } else if (size < 0x10000) {
    this._ch.writeByte(STRUCT_16);
    this._ch.writeByte(size/256>>0);
    this._ch.writeByte(size%256);
  } else {
...
Unpacker.prototype.unpack = function( buffer ) {
  ...
  if (marker >= 0 && marker < 128) {
    ...
    return readList(buffer.readByte());
  } else if (marker == LIST_16) {
    return readList(buffer.readUint16());
  } else if (marker == LIST_32) {
    return readList(buffer.readUint32());
